<template>
  <h1>Dryv Demo &mdash; HotChocolate GraphQL + Dryvue (Vue 3)</h1>
  <p>
    This demo uses a <strong>HotChocolate GraphQL</strong> backend with <strong>Dryvue</strong>
    for reactive client-side validation. The mutation is validated server-side by Dryv's
    HotChocolate type interceptor.
  </p>
...
const { validatable, validate } = useDryv(data, 'registration')
...
async function onSubmit() {
  ...
  try {
    const response = await fetch('/graphql', {
      ...
      body: JSON.stringify({
        query: `
          mutation Register($input: RegistrationInputInput!) {
    ...
    if (responseData.errors && responseData.errors.length > 0) {
      // Dryv validation errors come back as GraphQL errors with code DRYV_VALIDATION_ERROR
      const dryvErrors = responseData.errors.filter(e => e.extensions?.code === 'DRYV_VALIDATION_ERROR')
      if (dryvErrors.length > 0) {
        serverError.value = dryvErrors.map(e => e.message).join(', ')
      } else {
        serverError.value = responseData.errors[0].message
      }
    } else if (responseData.data?.register?.success) {
      successMessage.value = responseData.data.register.message
    }
  } finally {
