    ...
    internal class StringFormatDissector
    {
        ...
        public IList<object> Parse(string pattern)
        {
            ...
            foreach (var match in matches2)
            {
                ...
                result.Add((int.Parse(match.Groups[1].Value), match.Groups[2].Value?.TrimStart(':')));
            ...
            foreach (var part in this.Parse(pattern))
            {
                switch (part)
                {
                    case string text:
                        result.Add(text);
                        break;
                    case (int index, string format) when index < arguments.Length:
                        result.Add((arguments[index], format));
                        break;
                }
            }
